 ...
 * GOAT Plugin Runtime
 *
 * Dynamically loads all installed @goat-sdk plugins and exposes their tools.
 * Tools and parameters are fetched dynamically from the GOAT SDK.
 * 
 * The server wallet executes on behalf of users via x402 payments.
 */
...
async function doInitialize(): Promise<void> {
  ...
  try {
    ...
    console.log(`[goat] Wallet initialized on ${chain.name}: ${account.address}`);
    ...
    // ENS - Name resolution (requires mainnet provider)
    // Note: ENS plugin requires provider and chainId, skip for now on testnet
    ...
    console.log(`[goat] Loading ${plugins.length} plugins...`);
    ...
    for (const [toolName, tool] of Object.entries(toolsRecord)) {
      // Determine which plugin this tool belongs to (by naming convention)
      let pluginId = "wallet"; // Default to wallet tools
    ...
    console.log(`[goat] Loaded ${totalTools} tools from ${pluginRegistry.size} plugins`);
    for (const [id, plugin] of pluginRegistry) {
      console.log(`[goat]   - ${id}: ${plugin.toolCount} tools`);
    }
  } catch (err) {
    initError = err instanceof Error ? err.message : String(err);
    console.error("[goat] Failed to initialize:", initError);
  }
}
...
/**
 * Get runtime status with all plugins and tools
 */
...
export async function executeGoatTool(
  ...
  // Verify plugin matches (optional - allows flexibility)
  if (entry.pluginId !== pluginId && pluginId !== "any") {
    console.warn(`[goat] Tool ${toolName} belongs to plugin ${entry.pluginId}, not ${pluginId}`);
  }
  ...
  try {
    ...
    console.log(`[goat] ${toolName} completed`);
    if (txHash) console.log(`[goat] TX: ${txHash}`);

    return { success: true, result, txHash, gasUsed };
  } catch (err) {
    const errorMsg = err instanceof Error ? err.message : String(err);
    console.error(`[goat] ${toolName} failed:`, errorMsg);
