 ...
 */
export async function fetchGoogleModels(): Promise<GoogleModelInfo[]> {
    if (!GOOGLE_API_KEY) {
    ...
    try {
        const response = await fetch(
            `${BASE_URL}/v1beta/models?key=${GOOGLE_API_KEY}`
        );
        ...
        return data.models
            .filter((m) => m.supportedGenerationMethods?.some((method) => usableMethods.includes(method)))
            .map((model) => {
